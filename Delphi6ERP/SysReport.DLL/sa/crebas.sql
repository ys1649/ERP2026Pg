...
/*   Created on:     2002/12/16  01:48 ¤U¤ÈàŸ˜                  */
/* ============================================================ */
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('TBL_ADJ_DT')
            and   name  = 'TBL_ADJ_DT_PK'
            and   indid > 0
            and   indid < 255)
   drop index TBL_ADJ_DT.TBL_ADJ_DT_PK
go
...
if exists (select 1
           ...
           where  id    = object_id('TBL_ADJ_DT')
            ...
            and   indid > 0
            and   indid < 255)
   drop index TBL_ADJ_DT.REF_2343_PK
go
...
if exists (select 1
           ...
           where  id    = object_id('TBL_ADJ_DT')
            ...
            and   indid > 0
            and   indid < 255)
   drop index TBL_ADJ_DT.REF_PRD_ADD_PK
go
...
(
    ...
    SPR_TAX_RATE         numeric(18,4)         null    ,
...
VALUES('DEFAULT','Default Inventory','System Default','¨t²Î¦Û°Ê²£¥Í')
...
(
    ...
    constraint PK_TBL_ADJ_DT primary key (ADJ_NO, ADD_SEQNO)
)
go
...
/*   Index: REF_2343_PK                                         */
/* ============================================================ */
create index REF_2343_PK on TBL_ADJ_DT (ADJ_NO)
go

/* ============================================================ */
/*   Index: REF_PRD_ADD_PK                                      */
/* ============================================================ */
create index REF_PRD_ADD_PK on TBL_ADJ_DT (PRD_NO)
go
...
(
    ...
    RCD_QTY              numeric(18,4)         not null,
    RCD_UNIT_PRICE       money                 not null,
    constraint PK_TBL_HIS_PO_RECV_DT primary key (HRCV_NO, RCD_SEQNO)
)
go
...
alter table TBL_ADJ_DT
    add constraint FK_TBL_ADJ__REF_2343_TBL_INV_ foreign key  (ADJ_NO)
       references TBL_INV_ADJ (ADJ_NO)
go

alter table TBL_ADJ_DT
    add constraint FK_TBL_ADJ__REF_PRD_A_TBL_PROD foreign key  (PRD_NO)
       references TBL_PRODUCT (PRD_NO)
go
