...
function SendMessageToWindowName(WindowName : PChar;msg:Cardinal;wParam:integer) : boolean;
Function CloseWindowName(WindowName:string):boolean;
...
{
  wname1:=trim(string(WindowName));
  wname2:=trim(string(pchar(lParam)));
  r:=StrLIComp(pchar(wname1),pchar(wname2),10);
  if r = 0 then begin
    if wname1<>wname2 then begin
      showmessage('wname1='+wname1+',wname2='+wname2+' r='+inttostr(r));
    end;

    r:=StrLIComp(pchar(wname1),pchar(wname2),length(wname2));
    showmessage('wname1='+wname1+',wname2='+wname2+' r='+inttostr(r));
  end;
}
...
begin
  try
    ...
    If (WindowHandle=0) then EnumWindows(@EnumWindowsProc,Integer(PChar(WindowName)));
    If (WindowHandle<>0) then begin
      SendMessage(WindowHandle, msg, wParam, WindowHandle);
    end else BEGIN
      Result:=false;
    END;
  except
...
begin
  for i:= 1 to 1 do begin
    result:=SendMessageToWindowName(PChar(WindowName),WM_Close,0);
