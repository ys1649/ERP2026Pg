...
type
  TfrmRecChoice = class(TForm)
    ...
    procedure DsetBeforeScroll(DataSet: TDataSet);
    procedure DsetAfterScroll(DataSet: TDataSet);
  private
    MultiSelect:Boolean;
    KeyField:string;
    MarkList:TStringList;
    DatasetThroughList:TStringList;
...
begin
  ...
  fm.DatasetThroughList:=TStringList.Create;
end;

destructor TWRecChoice.Destroy;
begin
  fm.MarkList.Free;
  fm.DatasetThroughList.Free;
  fm.Free;
end;
...
var WasActive:Boolean;
    r :integer;
begin
//  fm.DataSource1.DataSet.BeforeScroll:=fm.DsetBeforeScroll;
  fm.DataSource1.DataSet.AfterScroll:=fm.DsetAfterScroll;
...
var i,n,cnt:integer;
    book,s,s1,s2,s3:string;
begin
  fm.MarkList.Clear;
    s1:=fm.DatasetThroughList.Strings[1];
    s2:=fm.DatasetThroughList.Strings[2];

    cnt:=fm.DatasetThroughList.Count;
    for i:= cnt-1 downto 0 do begin
      book:=fm.DatasetThroughList.Strings[i];
      fm.DataSource1.DataSet.GotoBookmark(pointer(book));
      s:=fm.DataSource1.DataSet.fieldbyname(fm.ResultField).AsString;
      n:=fm.DBGrid1.SelectedRows.IndexOf(book);
      if n<> -1 then begin
        if fm.MarkList.IndexOf(s)=-1 then begin
          fm.MarkList.insert(0,s);
        end;
      end;
    end;
end;
...
procedure TfrmRecChoice.DsetBeforeScroll(DataSet: TDataSet);
begin
  RecSelected;
end;

procedure TfrmRecChoice.DsetAfterScroll(DataSet: TDataSet);
var book:string;
begin
  book:=dataset.Bookmark;
  datasetThroughList.Add(book);
