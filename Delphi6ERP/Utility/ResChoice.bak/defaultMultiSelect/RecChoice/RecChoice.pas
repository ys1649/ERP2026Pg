...
begin
  cnt :=fm.DBGrid1.SelectedRows.Count;
	if (not MultiSelect) or (cnt=0) then //至少傳回一個,才不會導致空陣列
...
begin
  fm.MarkList.Clear;
  for i:=0 to fm.DBGrid1.SelectedRows.Count-1 do begin
    bookmark:=fm.DBGrid1.SelectedRows.Items[i];
    fm.DataSource1.DataSet.GotoBookmark(pointer(bookmark));
