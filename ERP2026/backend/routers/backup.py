...
@router.get("/export")
def export_backup():
    """匯出所有資料表結構+內容為單一 .sql 檔案（pg_dump plain 格式），供下載。"""
...
async def restore_backup(file: UploadFile):
    """還原：上傳備份 .sql 檔案 → 還原前先在伺服器留一份安全備份 → 用 psql 單一交易重建所有表並灌回資料。"""
    import subprocess
    ...
    try:
        ...
        ts = datetime.now().strftime("%Y%m%d%H%M%S")
        safety_filename = f"pre_restore_{ts}.sql"
