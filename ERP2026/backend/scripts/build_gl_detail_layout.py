"""手動重建 Form_AcntDetail.dfm 的 ppReport 版面，寫回 GL_DETAIL（SRP_ID=67）的 SRP_REPORTFILE。
版面結構跟 build_gl_cash_layout.py（現金簿）幾乎一樣（原始 .dfm 座標也幾乎一樣），差異只在
這份報表沒有寫死科目，ppGroup1（BreakName=ACT_NO）會真的依科目分很多組，「餘額」欄位
（ResetGroup=ppGroup1）也真的會在換科目時歸零重算——所以後端 SQL 用 window function 算
running balance 時要加 PARTITION BY ACT_NO（現金簿因為永遠只有一個科目，不需要 PARTITION BY）。
...
def build_tree():
    header = band("TppHeaderBand", "HeaderBand1", 44450, [
        ...
        line("Line2", 34660),
        label("Label1", "明細分類帳", 87842, 21696, 21167, align="taCentered"),
...
def main():
    ...
    report, warnings = convert_report(build_tree(), "GL_DETAIL", "明細分類帳", root_columns)
