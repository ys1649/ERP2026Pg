...
import { DollarOutlined, SearchOutlined, CloseOutlined, RollbackOutlined } from '@ant-design/icons'
...
// GL_CASH 這筆系統報表定義的 SRP_ID（backend/scripts/create_gl_reports.py 建立，ID 已固定不會變動），
// 這裡只借用它存的 Stimulsoft 版面（SRP_REPORTFILE），資料來源改走專屬的 /api/gl-reports/cash。
const GL_CASH_SRP_ID = 66
const VIEWER_ID = 'sti-cash-book-viewer'
...
export default function CashBook() {
  ...
  useEffect(() => {
    ...
    // 預設值：傳票起始日期=會計年度/1/1，傳票終止日期=會計年度/12/31（比照 From_AcntCash.pas 的 init()）
    glReportsApi.cashDefaults().then((res) => {
      form.setFieldsValue({
  ...
  const handlePreview = async () => {
    ...
    try {
      const [reportRes, dataRes] = await Promise.all([
        sysReportApi.getReportFile(GL_CASH_SRP_ID),
        glReportsApi.cash(dtFrom, dtTo),
      ])
      ...
      const viewer = new S.Viewer.StiViewer(viewerOptions, 'CashBookViewer', false)
  ...
  const message_warning = () => {
    notification.warning({ message: '尚未設計版面', description: '請先至「系統報表管理」找 GL_CASH，點「設計」建立版面。', duration: 5 })
  }
      ...
      <Card style={{ maxWidth: 420, margin: '40px auto' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>現金簿</Title>
          </Space>
