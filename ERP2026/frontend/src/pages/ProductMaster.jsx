...
export default function ProductMaster() {
  ...
  const fetchData = useCallback(async (params = searchParams, pg = page, ps = pageSize) => {
    ...
    try {
      const res = await productApi.list({ ...params, page: pg, page_size: ps })
  ...
  const handleFormOk = async (values) => {
    ...
    try {
      if (editRecord) {
        await productApi.update(editRecord.prd_no, values)
  ...
  const columns = [
    {
      title: '產品編號', dataIndex: 'prd_no', key: 'prd_no', width: 100,
      render: (v) => <Tag color="blue">{v}</Tag>,
    },
    {
      title: '產品名稱', dataIndex: 'prd_name', key: 'prd_name', ellipsis: true,
      render: (v) => <Tooltip title={v}>{v}</Tooltip>,
    },
    { title: '單位', dataIndex: 'prd_unit', key: 'prd_unit', width: 70 },
    { title: '售價', dataIndex: 'prd_sale_price', key: 'prd_sale_price', width: 100, align: 'right' },
    { title: '安全存量', dataIndex: 'prd_safe_qty', key: 'prd_safe_qty', width: 100, align: 'right' },
    { title: '現有庫存', dataIndex: 'prd_onhand', key: 'prd_onhand', width: 100, align: 'right' },
    { title: '現行成本', dataIndex: 'prd_cur_cost', key: 'prd_cur_cost', width: 100, align: 'right' },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          <Popconfirm
            title={`確定刪除「${record.prd_name}」？`}
            okText="刪除" cancelText="取消" okButtonProps={{ danger: true }}
            onConfirm={() => handleDelete(record.prd_no)}
          >
    ...
    <>
        ...
        <Row justify="space-between" align="middle" gutter={[16, 8]} wrap>
          <Col flex="none">
            <Space direction="vertical" size={0}>
              <Title level={4} style={{ margin: 0 }}>產品基本資料設定</Title>
      ...
      <Card styles={{ body: { padding: '8px 16px' } }}>
        <Table
          ...
          size="small"
          scroll={{ x: 970, y: 'calc(100vh - 400px)' }}
          pagination={{
            current: page,
            pageSize,
            total,
