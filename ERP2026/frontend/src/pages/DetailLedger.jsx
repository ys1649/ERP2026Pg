...
import { Card, Form, DatePicker, Input, Button, Space, Typography, Modal, Spin, notification } from 'antd'
import { FileTextOutlined, SearchOutlined, CloseOutlined, RollbackOutlined } from '@ant-design/icons'
...
// GL_DETAIL 這筆系統報表定義的 SRP_ID（backend/scripts/create_gl_reports.py 建立，ID 已固定不會變動），
// 這裡只借用它存的 Stimulsoft 版面（SRP_REPORTFILE），資料來源改走專屬的 /api/gl-reports/detail。
const GL_DETAIL_SRP_ID = 67
const VIEWER_ID = 'sti-detail-ledger-viewer'
...
export default function DetailLedger() {
  ...
  useEffect(() => {
    ...
    // 預設值：傳票起始日期=會計年度/1/1，傳票終止日期=會計年度/12/31（比照 Form_AcntDetail.pas 的 SetupQueryForm）
    glReportsApi.detailDefaults().then((res) => {
      form.setFieldsValue({
  ...
  const handlePreview = async () => {
    ...
    try {
      const [reportRes, dataRes] = await Promise.all([
        sysReportApi.getReportFile(GL_DETAIL_SRP_ID),
        glReportsApi.detail(dtFrom, dtTo, values.act_no_from, values.act_no_to),
      ])
      ...
      metaDataSet.readJson(JSON.stringify({
        srpmeta: [{
          srp_id: GL_DETAIL_SRP_ID,
          srp_code: 'GL_DETAIL',
          srp_name: '明細分類帳',
          srp_description: `傳票期間 ${dataRes.data.dt_range}`,
      ...
      const viewer = new S.Viewer.StiViewer(viewerOptions, 'DetailLedgerViewer', false)
  ...
  const message_warning = () => {
    notification.warning({ message: '尚未設計版面', description: '請先至「系統報表管理」找 GL_DETAIL，點「設計」建立版面。', duration: 5 })
  }
      ...
      <Card style={{ maxWidth: 420, margin: '40px auto' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>明細分類帳</Title>
          </Space>
          <Form form={form} layout="horizontal" labelCol={{ span: 9 }}>
            ...
            <Form.Item name="act_no_from" label="科目編號（起）">
              <Input placeholder="選填，留空=不篩選" allowClear />
            </Form.Item>
            <Form.Item name="act_no_to" label="科目編號（迄）">
              <Input placeholder="選填，留空=不篩選" allowClear />
            </Form.Item>
          </Form>
