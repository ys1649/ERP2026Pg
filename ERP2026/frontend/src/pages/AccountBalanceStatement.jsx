...
// GL_BALANCE 這筆系統報表定義的 SRP_ID（backend/scripts/create_gl_reports.py 建立，ID 已固定不會變動），
// 這裡只借用它存的 Stimulsoft 版面（SRP_REPORTFILE），資料來源改走專屬的 /api/gl-reports/balance。
const GL_BALANCE_SRP_ID = 63
const VIEWER_ID = 'sti-account-balance-viewer'
...
export default function AccountBalanceStatement() {
  ...
  useEffect(() => {
    ...
    // 預設值：查詢截止日期=會計年度(TBL_SYS_PARAM.SPR_ACNT_YEAR)/12/31（比照 Form_AcntBalance.pas 的 init()）
    glReportsApi.balanceDefaults().then((res) => {
      form.setFieldsValue({ dt_end: dayjs(res.data.dt_end) })
    }).catch(() => {})
  ...
  const handlePreview = async () => {
    ...
    try {
      const [reportRes, dataRes] = await Promise.all([
        sysReportApi.getReportFile(GL_BALANCE_SRP_ID),
        glReportsApi.balance(dtEnd),
      ])
      ...
      metaDataSet.readJson(JSON.stringify({
        srpmeta: [{
          srp_id: GL_BALANCE_SRP_ID,
          srp_code: 'GL_BALANCE',
          srp_name: '科目餘額表',
          srp_description: `查詢截止日期 ${dataRes.data.dt_end}`,
      ...
      const viewer = new S.Viewer.StiViewer(viewerOptions, 'AccountBalanceViewer', false)
  ...
  const message_warning = () => {
    notification.warning({ message: '尚未設計版面', description: '請先至「系統報表管理」找 GL_BALANCE，點「設計」建立版面。', duration: 5 })
  }
      ...
      <Card style={{ maxWidth: 420, margin: '40px auto' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>科目餘額表</Title>
          </Space>
