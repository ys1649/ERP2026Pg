...
// GL_DAILY 這筆系統報表定義的 SRP_ID（backend/scripts/create_gl_reports.py 建立，ID 已固定不會變動），
// 這裡只借用它存的 Stimulsoft 版面（SRP_REPORTFILE），資料來源改走專屬的 /api/gl-reports/daily。
const GL_DAILY_SRP_ID = 64
const VIEWER_ID = 'sti-daily-journal-viewer'
...
export default function DailyJournal() {
  ...
  useEffect(() => {
    ...
    // 預設值：傳票起始日期=會計年度/1/1，傳票終止日期=會計年度/12/31（比照 Form_AcntDaily.pas 的 init()）
    glReportsApi.dailyDefaults().then((res) => {
      form.setFieldsValue({
  ...
  const handlePreview = async () => {
    ...
    try {
      const [reportRes, dataRes] = await Promise.all([
        sysReportApi.getReportFile(GL_DAILY_SRP_ID),
        glReportsApi.daily(dtFrom, dtTo),
      ])
      ...
      const viewer = new S.Viewer.StiViewer(viewerOptions, 'DailyJournalViewer', false)
  ...
  const message_warning = () => {
    notification.warning({ message: '尚未設計版面', description: '請先至「系統報表管理」找 GL_DAILY，點「設計」建立版面。', duration: 5 })
  }
      ...
      <Card style={{ maxWidth: 420, margin: '40px auto' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>日記帳</Title>
          </Space>
