...
import { TableOutlined, SearchOutlined, CloseOutlined, RollbackOutlined } from '@ant-design/icons'
...
// GL_TRIAL 這筆系統報表定義的 SRP_ID（backend/scripts/create_gl_reports.py 建立，ID 已固定不會變動），
// 這裡只借用它存的 Stimulsoft 版面（SRP_REPORTFILE），資料來源改走專屬的 /api/gl-reports/trial。
const GL_TRIAL_SRP_ID = 65
const VIEWER_ID = 'sti-trial-balance-viewer'
...
export default function TrialBalance() {
  ...
  useEffect(() => {
    ...
    // 預設值：起訖日期都預設系統日期（比照 Form_AcntTrialBalance.pas 的 init()）
    glReportsApi.trialDefaults().then((res) => {
      form.setFieldsValue({
  ...
  const handlePreview = async () => {
    ...
    try {
      const [reportRes, dataRes] = await Promise.all([
        sysReportApi.getReportFile(GL_TRIAL_SRP_ID),
        glReportsApi.trial(dtFrom, dtTo),
      ])
      ...
      metaDataSet.readJson(JSON.stringify({
        srpmeta: [{
          srp_id: GL_TRIAL_SRP_ID,
          srp_code: 'GL_TRIAL',
          srp_name: '試算表',
          srp_description: `查詢期間 ${dataRes.data.dt_range}`,
      ...
      const viewer = new S.Viewer.StiViewer(viewerOptions, 'TrialBalanceViewer', false)
  ...
  const message_warning = () => {
    notification.warning({ message: '尚未設計版面', description: '請先至「系統報表管理」找 GL_TRIAL，點「設計」建立版面。', duration: 5 })
  }
      ...
      <Card style={{ maxWidth: 420, margin: '40px auto' }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          <Space>
            ...
            <Title level={4} style={{ margin: 0 }}>試算表</Title>
          </Space>
